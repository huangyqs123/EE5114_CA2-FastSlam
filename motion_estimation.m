...
for i = 1:size(output_temp,1)-1
	...
	if (assoc_count >= 2)
        ...
        p_centroid = mean(P, 1); % Compute the centroid of P
        q_centroid = mean(Q, 1); % Compute the centroid of Q
        P_new = P - p_centroid; % Subtract p_centroid from each row of P
        Q_new = Q - q_centroid; % Subtract q_centroid from each row of Q
        P_new = P_new'; %Transpose matrix
        Q_new = Q_new';
        ...
        % Calculate the sign of determinant of V*U'
        d = sign(det(V * U'));
