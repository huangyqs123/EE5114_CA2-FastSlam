	...
	for i = 1:size(lines, 2)-1
		if (lines(i).p2.x == lines(i+1).p1.x && lines(i).p2.y == lines(i+1).p1.y)
			...
			if (lines(i).length > length_min && lines(i+1).length > length_min)
                ...
                va_x = lines(i+1).p2.x - lines(i).p2.x;
                va_y = lines(i+1).p2.y - lines(i).p2.y;
                
                vb_x = lines(i).p1.x - lines(i+1).p1.x;
                vb_y = lines(i).p1.y - lines(i+1).p1.y;

                % Calculate angle span of the corner 
                angle = slam_in_pi(acos((va_x * vb_x + va_y * vb_y) / (norm([va_x, va_y]) * norm([vb_x, vb_y]))));
				...
				if (angle > deg2rad(60) && angle < deg2rad(120))
                    ...
                    va = [va_x, va_y];
                    vb = [vb_x, vb_y];
                    value_xy = va/norm(va) + vb/norm(vb);
                    x = value_xy(1);
                    y = value_xy(2);
                    heading = slam_in_pi(atan2(y, x)+pi);
